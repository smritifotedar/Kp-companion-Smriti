...
import { geocode, type GeoCity } from '@/lib/geo';
...
export function CityAutocomplete({
  ...
  return (
    <div className="relative">
      ...
      />
      {loading && <Loader2 size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-saffron-400 animate-spin" />}
        ...
        <div className="absolute z-30 left-0 right-0 mt-1 bg-white border border-earth-200 rounded-xl shadow-premium-lg max-h-56 overflow-y-auto">
          {results.length === 0 ? (
            ...
            results.map((c, i) => (
              <button
                key={`${c.name}-${c.lat}-${i}`}
                onMouseDown={(e) => { e.preventDefault(); onSelect(c); setQuery(`${c.name}, ${c.region}`); setOpen(false); }}
